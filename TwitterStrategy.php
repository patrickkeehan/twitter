...
class TwitterStrategy extends OpauthStrategy {
	...
	public function oauth_callback() {
		...
		if (!empty($_REQUEST['oauth_token']) && $_REQUEST['oauth_token'] == $_COOKIE['oauth_token']) {
			$this->tmhOAuth->config['user_token'] = $_COOKIE['oauth_token'];
			$this->tmhOAuth->config['user_secret'] = $_COOKIE['oauth_token_secret'];
	...
	private function _verify_credentials($user_token, $user_token_secret) {
		...
		$params = array(
			'skip_status' => $this->strategy['verify_credentials_skip_status'],
			'include_email' => 'true'
		);
